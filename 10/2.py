...
# for all unvisited points : check if a ray intersect count will be odd or even
# run the ray diagonally to avoid the problem of aligning the ray with a pipe
# if we encounter L or 7 we discount the intersection
cnt = 0
for pos in g.keys():
    if pos in visited:
        continue
    ...
    while x <= mx and y <= my:
        tile = g[(x,y)]
        if (x, y) in visited and tile not in ["L", "7"]:
            cross += 1
        x += 1
        y += 1
    if cross % 2 == 1:
